...
ImageTextAsciifyAPI_applyImageTextAsciifyPost(apiClient_t *apiClient ,binary_t* image ,model_e model)
{
    ...
    // create the path
    long sizeOfPath = strlen("/image/text/asciify/")+1;
    char *localVarPath = malloc(sizeOfPath);
    snprintf(localVarPath, sizeOfPath, "/image/text/asciify/");
    ...
    cJSON *ImageTextAsciifyAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
    object_t *elementToReturn = object_parseFromJSON(ImageTextAsciifyAPIlocalVarJSON);
    cJSON_Delete(ImageTextAsciifyAPIlocalVarJSON);
