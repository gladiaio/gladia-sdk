...
feature -- API Access
	...
	apply_image_text_asciify_post (image: FILE; model: STRING_32): detachable ANY
			-- Apply model for the asciify task for a given models
			-- 
			-- 
			-- argument: image  (required)
			-- 
			-- argument: model  (optional, default to ramesh-aditya)
			-- 
			-- 
			-- Result ANY
		require
	...
	versions_image_text_asciify_get : detachable ANY
